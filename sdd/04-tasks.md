...
Acceptance pass: `npm run acceptance` → **16/16 checks PASS** against the real catalog on
the current TypeScript code (re-run 2026-09-14; see `05-acceptance-report.md`).
...
- [x] 2.5 TRIANGULATE: run the full real catalog through both scripts; verify record
      ...
      **Caveat:** verified by inspecting committed artifacts; no automated test asserts
      count/uniqueness against the real catalog (tracked as 2.7).
...
- [ ] 2.7 *(added)* Add an automated assertion over the real catalog for record count and
      id uniqueness. Today 2.5 is a manual artifact check, so a normalize/generate
      regression that drops or duplicates records would pass CI.
...
- [x] 7.4 Final acceptance checklist AC-1..AC-7; note results in the verify report.
      — Evidence: `sdd/05-acceptance-report.md` — 16/16 pass, reproducible; AC-6 verified
      live.
...
(user-supplied tricky-search list) and the optional hardening item 2.7 (automated count /
